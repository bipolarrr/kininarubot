...
- `/queue`: 현재 곡과 전체 대기열을 10개 단위 페이지로 표시
- `/remove index:<number>`: `/queue`에 표시된 대기열 번호 삭제
...
검색어는 `yt-dlp`의 `ytsearch5:` 결과를 가져온 뒤 검색어와 제목의 일치도, official audio, official music video, Topic 채널 같은 제목/채널 신호를 점수화해 가장 의도에 가까운 후보를 고릅니다. 검색어에 remix, cover, live 같은 버전 키워드를 명시하면 해당 키워드가 제목에 있는 후보를 원곡보다 우선합니다. 검색은 빠른 후보 조회를 위해 flat playlist metadata를 사용하므로 YouTube Data API 수준의 공식성을 보장하지는 않습니다.

`실지주 1기 op`, `실지주 2기 엔딩`처럼 애니 제목/약칭, 시즌, OP/ED 키워드가 들어간 자동 주제곡 검색은 검색 방식을 다시 정할 때까지 잠시 꺼져 있습니다. 이 경우 재생을 시작하지 않고 곡명이나 아티스트를 함께 입력해 달라는 메시지를 반환합니다.
