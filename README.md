...
검색어는 URL이 아니면 `lyrics` 또는 `가사`가 이미 포함된 경우를 제외하고 끝에 `lyrics`를 자동으로 붙인 뒤 `yt-dlp`의 `ytsearch20:` 결과를 가져옵니다. 봇은 별도 점수화 없이 YouTube가 반환한 순서대로 제목과 URL이 있는 후보를 확인하고, 첫 재생 가능 후보를 재생합니다. 검색은 빠른 후보 조회를 위해 flat playlist metadata를 사용하므로 YouTube Data API 수준의 공식성을 보장하지는 않습니다.
